...
test('AC1: installSkills instala exatamente as 5 skills reais (condutor + 4 especialistas)', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      installed,
      ['process-ai', 'process-ai-bento', 'process-ai-julia', 'process-ai-miguel', 'process-ai-zanoni'],
      'installSkills deve instalar exatamente as 5 skills com SKILL.md',
    );
  } finally {
    await fs.rm(tmp, { recursive: true, force: true });
  }
